<template>
  <div>
    ...
    >
      <Form :validation-schema="deckSchema" @submit="createDeck">
        ...
        <Field name="title" v-slot="{ field, errors }">
          ...
          />
          <p v-if="errors[0]" class="mt-1 text-sm text-red-400">
            {{ errors[0] }}
          </p>
        </Field>
    ...
    >
      ...
      <div
        v-if="selectedDeckIds.length"
        class="flex flex-wrap items-center gap-2"
      >
        <span class="text-sm text-slate-400"
          >{{ selectedDeckIds.length }} selected</span
        >
        <button
          type="button"
    ...
    <p v-else-if="!pending" class="mt-8 text-center text-slate-500">
...
const createDeck: SubmissionHandler = async (values) => {
  const title = String(values.title ?? "");
  try {
    await api("/decks", { method: "POST", body: { title } });
...
async function executeDeckDelete() {
  ...
  selectedDeckIds.value = selectedDeckIds.value.filter(
    (id) => !ids.includes(id),
  );
  await fetchDecks();
}
